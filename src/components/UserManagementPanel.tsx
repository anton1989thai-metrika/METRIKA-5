...
export default function UserManagementPanel() {
  ...
  const updateUser = async (userId: string, updates: Partial<UserType>) => {
    ...
    const r = await syncUsersWithServer(updatedUsers);
    if (!r.ok) {
      // rollback local state
      setUsers(prevUsers)
      alert(r.error || 'Не удалось сохранить изменения')
      return false
    }
    // close editor handled by caller
    return true
  }
  ...
  const deleteUser = async (userId: string) => {
    if (confirm('Вы уверены, что хотите удалить этого пользователя?')) {
      ...
      if (!r.ok) {
        setUsers(prevUsers)
        alert(r.error || 'Не удалось удалить пользователя')
      }
    }
  }
  ...
  const toggleUserStatus = async (userId: string) => {
    ...
    if (!r.ok) {
      setUsers(prevUsers)
      alert(r.error || 'Не удалось изменить статус')
    }
  }
  ...
  const saveIndividualPermissions = async () => {
    ...
    // Вызываем событие для обновления меню у текущего пользователя
    if (typeof window !== 'undefined') {
      window.dispatchEvent(new CustomEvent('permissionsUpdated', { 
        detail: { userId: selectedUserForPermissions.id } 
      }));
    }
                ...
                <div className="bg-white shadow-sm border border-gray-300 rounded-lg p-6">
                  ...
                  {selectedUser?.needsPassword ? (
                    <div className="text-xs text-red-600 mb-2">
                      Пароль не задан — пользователь не сможет войти, пока вы не установите новый пароль.
                    </div>
                  ) : null}
                  <Input
                    type="text"
            ...
            <Button 
              onClick={async () => {
                ...
                if (passwordDraft && passwordDraft.trim().length > 0) {
                  ...
                  // do not send password when not changing
                  delete updates.password
                }
                const ok = await updateUser(selectedUser.id, updates)
                if (!ok) return
                setPasswordDraft('')
                setIsEditingUser(false)
                setSelectedUser(null)
